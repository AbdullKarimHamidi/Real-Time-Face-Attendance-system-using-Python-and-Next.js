...
// import { Search, Radio, ShieldCheck, Activity, UserCheck, AlertCircle } from "lucide-react";
...
//   // Memoized fetcher for camera list
...
//       setError("Unable to connect to camera server. Please ensure the backend is active.");
//     } finally {
//       setLoading(false);
//     }
//   }, []);
...
//   // Effect 2: Recognition Interval
//   useEffect(() => {
//     const fetchRecognized = async () => {
//       try {
//         const res = await fetch(`${BASE_URL}/recognized`);
//         if (res.ok) setRecognized(await res.json());
//       } catch (err) {
//         console.warn("Recognition service unreachable");
//       }
//     };
...
//   if (loading) {
//     return (
//       <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#020617] gap-4">
//         <Activity className="w-10 h-10 text-cyan-500 animate-spin" />
//         <p className="text-sm font-black uppercase tracking-widest dark:text-white">Connecting to Node Network...</p>
//       </div>
//     );
//   }
...
//   return (
//     <div className="min-h-screen w-full bg-slate-50 dark:bg-[#020617] p-4 transition-colors duration-500">
//       {/* Header and Search ... (same as your previous code) */}
      
//       <main className="w-full">
//         {error && (
//           <div className="bg-red-500/10 border border-red-500/50 p-4 rounded-2xl mb-6 flex items-center gap-3 text-red-500">
//             <AlertCircle className="w-5 h-5" />
//             <span className="text-sm font-bold">{error}</span>
//             <button onClick={fetchCameras} className="ml-auto underline font-black text-xs uppercase">Retry</button>
//           </div>
//         )}

//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
//           {cameras.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase())).map((cam) => (
//             <div key={cam.name} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 rounded-[2.5rem] overflow-hidden shadow-xl">
//               {/* Camera Content */}
//               <div className="relative aspect-video m-4 rounded-[1.8rem] overflow-hidden bg-slate-800">
//                 <img
//                   src={cam.url}
//                   alt={cam.name}
//                   className="w-full h-full object-cover"
//                   onError={(e) => { (e.target as HTMLImageElement).src = "https://placehold.co/600x400/1e293b/white?text=Node+Offline"; }}
//                 />
//                 {/* HUD Overlay logic... */}
//               </div>
//             </div>
...
import Image from "next/image";
import { useEffect, useState, useCallback } from "react";
import { Search, Radio, ShieldCheck, Activity, UserCheck, AlertTriangle } from "lucide-react";
...
export default function LiveCamerasPage() {
  ...
  useEffect(() => {
    const fetchRecognized = async () => {
      try {
        ...
        console.error("Recognition sync failed");
      }
    };
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 dark:bg-[#020617] p-4 transition-colors duration-500">
      ...
      <main className="w-full">
        {error && (
          <div className="flex items-center justify-center p-10 text-red-500 gap-3 border border-red-500/20 bg-red-500/5 rounded-[2rem] mb-6">
            <AlertTriangle />
            <span className="font-bold uppercase tracking-widest">Node Connection Error: {error}</span>
            <button onClick={fetchCameras} className="ml-4 underline hover:text-red-400">Retry Connection</button>
          </div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-2">
          {cameras.filter(cam => cam.name.toLowerCase().includes(searchQuery.toLowerCase())).map((cam) => {
            const person = recognized[cam.name];
            return (
              <div key={cam.name} className="group relative bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/5 rounded-[2.5rem] overflow-hidden shadow-xl transition-all duration-500 hover:shadow-cyan-500/10 hover:border-cyan-500/30">
                <div className="px-6 py-4 flex justify-between items-center bg-slate-50/50 dark:bg-white/5 border-b border-slate-100 dark:border-white/5">
                  <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                    <Radio className="w-3 h-3 text-red-500 animate-pulse" /> {cam.name} Node
                  </h3>
                  <ShieldCheck className="w-4 h-4 text-emerald-500 opacity-50" />
                </div>

                <div className="relative aspect-video mx-4 mt-4 rounded-[1.8rem] overflow-hidden bg-slate-200 dark:bg-slate-800">
                  <img
                    src={cam.url}
                    alt={cam.name}
                    className="w-full h-full object-cover"
                    onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=1000"; }}
                  />
                  {person && person.name !== "Unknown" && (
                    <div className="absolute inset-x-3 bottom-3 bg-black/60 backdrop-blur-xl border border-white/20 text-white rounded-2xl p-3 flex items-center gap-4">
                      <img src={`${BASE_URL}${person.image}`} alt={person.name} className="w-10 h-10 rounded-lg object-cover" />
                      <div>
                        <p className="text-xs font-black uppercase">{person.name}</p>
                        <p className="text-[9px] text-slate-300">{person.lastName}</p>
                      </div>
                    </div>
                  )}
                </div>
